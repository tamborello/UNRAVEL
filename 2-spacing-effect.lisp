...
;;; 
;;; Bugs        : * Floating point overflow occurs in compute-spacing-effect-
;;;		activation
;;;
;;; To do       : 
;;; 
...
;;; 2013.10.16 fpt 2
;;;		: * Limit time and decay that goes into spacing effect activation
;;;		because I keep getting floating point overflow errors.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
;;; 
    ...
    (if (= (length (chunk-reference-list chunk))
        ...
        (progn
          (mapcar #'(lambda (reference decay)
                        (incf value 
                            (expt (max .05 (min max-time (ms->seconds (- ct reference))))
                                          (- (min max-decay decay)))))
